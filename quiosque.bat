...
:: --- Escreve o script PowerShell num temporário ---
...
echo $args = "--app=`"$kioskUrl`" --user-data-dir=`"$profileDir`" --start-fullscreen --no-first-run --no-default-browser-check --disable-infobars --disable-session-crashed-bubble --disable-restore-session-state --disable-translate --disable-features=TranslateUI --disable-extensions --kiosk-printing"
echo.
...
echo $sc.Save^(^)
echo.
echo # --- Atalho na pasta Inicializar (abre junto com o Windows) ---
...
echo $sc2.Save^(^)
echo.
echo Write-Host 'Instalacao concluida. Abrindo o quiosque...'
echo Start-Process $chrome -ArgumentList $args
) > "%PS%"
