/* ─── DEV RULER v2 ────────────────────────────────────────────
   ...
   Funcionalidades:
   • Arrastar guias com precisão de 1 px (área de toque 28 px)
   • Ajuste fino ±1 / ±10 px ao selecionar uma guia
   • Trocar a cor de cada guia individualmente (6 cores)
   • Salvar e carregar esquemas no localStorage
   ─────────────────────────────────────────────────────────────── */
...
(function () {
  ...
  function putSchemes(obj) { localStorage.setItem(STORAGE_KEY, JSON.stringify(obj)); }
  ...
  function nudge(delta) {
    if (!selected) return;
    selected.pos = Math.max(0, selected.pos + delta);
  ...
  nudgeRow.append(bM10, bM1, posLabel, bP1, bP10, bDel);

  /* Swatches de cor da guia selecionada */
  ...
  function syncGuideDOM(g) {
    if (g.type === 'h') {
      g.line.style.top  = g.pos + 'px';
      g.hit.style.top   = (g.pos - 14) + 'px';
    } else {
      g.line.style.left = g.pos + 'px';
      g.hit.style.left  = (g.pos - 14) + 'px';
    }
    g.badge.textContent = g.pos + 'px';
  }

  function addGuide(type, pos, colorDef) {
    pos = Math.round(pos);
    colorDef = colorDef || currentColor;

    const line  = document.createElement('div');
    const hit   = document.createElement('div'); // área de toque larga
    ...
    if (type === 'h') {
      Object.assign(line.style, {
        ...
        background: colorDef.line, pointerEvents: 'none',
      });
  ...
  /* ── Tap layer (coloca novas guias) ──────────────────────────── */
  const tapLayer = document.createElement('div');
  Object.assign(tapLayer.style, {
    position: 'fixed', inset: '0', zIndex: '8995', display: 'none',
  });
  ...
  mSaveOK.addEventListener('click', () => {
    ...
    schemes[name] = guides.map(g => ({ type: g.type, pos: g.pos, colorId: g.colorDef.id }));
    putSchemes(schemes);
      ...
      if (!names.length) {
        ...
        names.forEach(name => {
          ...
          delBtn.addEventListener('click', () => {
            const s = getSchemes(); delete s[name]; putSchemes(s);
            row.remove();
          });
  ...
  /* Indicador de cor atual (ponto colorido sobre o botão H) */
  ...
  /* Mini painel de seleção de cor (abre ao segurar H ou V) */
  ...
  /* Segurar H ou V por 500 ms abre o seletor de cor */
  let holdTimer;
  [btnH, btnV].forEach(b => {
    b.addEventListener('touchstart', () => {
      holdTimer = setTimeout(() => { colorPick.style.display = colorPick.style.display === 'flex' ? 'none' : 'flex'; }, 500);
    }, { passive: true });
  ...
  panel.append(btnLod, btnSav, btnClr, btnV, btnHWrap);
  ...
  document.body.append(canvas, guideLayer, tapLayer, panel, colorPick, selPanel, modal, toggleBtn);
})();
